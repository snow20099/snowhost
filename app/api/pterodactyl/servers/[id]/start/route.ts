...
export async function POST(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Pterodactyl start server error:', errorData)
      return NextResponse.json({ 
        error: errorData.errors?.[0]?.detail || 'فشل في تشغيل الخادم'
      }, { status: response.status })
    }

    return NextResponse.json({ 
      message: 'تم تشغيل الخادم بنجاح',
      serverId 
    })
